...
void main() {
  group('CodeLines constructor ', () {
    ...
    test('`CodeLines.of()`', () {
      ...
      }
      {
        final CodeLines codeLines = CodeLines.of(
          List.generate(512, (index) => CodeLine('$index')),
        );
    ...
    test('`CodeLines.from()`', () {
      ...
      {
        final CodeLines codeLines = CodeLines.from(
          const CodeLines([
            CodeLineSegment(
              codeLines: [CodeLine('abc'), CodeLine('foo'), CodeLine('bar')],
            ),
      ...
      }
      {
        ...
        expect(
          listEquals(codeLines.segments, const [
            ...
            CodeLineSegment(codeLines: [CodeLine('foo')], dirty: true),
            CodeLineSegment(codeLines: [CodeLine('bar')], dirty: true),
          ]),
          true,
        );
      }
  ...
  group('CodeLines getter ', () {
    test('`first` and `last`', () {
      ...
      {
        final CodeLines codeLines = CodeLines.of(
          List.generate(3000, (index) => CodeLine('$index')),
        );
    ...
    test('`lineCount`', () {
      ...
      {
        final CodeLines codeLines = CodeLines.of(const [
          CodeLine('abc', [CodeLine('123'), CodeLine('456'), CodeLine('789')]),
          CodeLine('foo', [CodeLine('123'), CodeLine('456'), CodeLine('789')]),
          CodeLine('bar', [CodeLine('123'), CodeLine('456'), CodeLine('789')]),
        ]);
    ...
    test('`[]=`', () {
      {
        final CodeLines codeLines = CodeLines.empty();
        expect(
          () => codeLines[0] = const CodeLine('abc'),
      ...
      }
      {
        ...
        expect(
          () => codeLines[1] = const CodeLine('abc'),
          throwsA(isA<RangeError>()),
        );
      }
      {
  ...
  group('CodeLines method ', () {
    test('`add()`', () {
      ...
      {
        final CodeLines codeLines = CodeLines.of(
          List.generate(1024, (index) => CodeLine('$index')),
        );
        codeLines.add(const CodeLine('abc'));
      ...
      }
      {
        final CodeLines codeLines = CodeLines([
          CodeLineSegment(
            codeLines: List.of(const [CodeLine('abc')]),
            dirty: true,
          ),
        ]);
        codeLines.add(const CodeLine('foo'));
    ...
    test('`addFrom()`', () {
      ...
      }
      {
        ...
        final CodeLines codeLines2 = CodeLines([
          CodeLineSegment(codeLines: List.of([const CodeLine('123')])),
        ]);
        codeLines2.addFrom(codeLines1, 1, 2);
        expect(
          listEquals(codeLines2.segments, const [
            CodeLineSegment(codeLines: [CodeLine('123'), CodeLine('foo')]),
          ]),
          true,
        );
      ...
      }
      {
        ...
        final CodeLines codeLines2 = CodeLines.of(
          List.generate(512, (index) => CodeLine('$index')),
        );
        codeLines1.addFrom(codeLines2, 0);
        expect(
      ...
      }
      {
        final CodeLines codeLines1 = CodeLines.of(const [CodeLine('abc')]);
        final CodeLines codeLines2 = CodeLines([
          const CodeLineSegment(codeLines: [CodeLine('foo')]),
          const CodeLineSegment(codeLines: [CodeLine('bar')]),
          CodeLineSegment(
            codeLines: List.generate(256, (index) => CodeLine('$index')),
          ),
          const CodeLineSegment(codeLines: [CodeLine('abc')]),
    ...
    test('`asString`', () {
      ...
      }
      {
        ...
        expect(
          codeLines.asString(TextLineBreak.crlf),
          'abc\r\n123\r\n456\r\n789\r\nfoo\r\n123\r\n456\r\n789\r\nbar\r\n123\r\n456\r\n789',
        );
        expect(
          codeLines.asString(TextLineBreak.cr),
    ...
    test('`equals()`', () {
      ...
      expect(
        CodeLines.empty().equals(CodeLines.of(const [CodeLine('abc')])),
        false,
      );
      expect(
        CodeLines.of(const [CodeLine('abc')]).equals(CodeLines.empty()),
        false,
      );
      expect(
        CodeLines.of(const [
    ...
    test('`sublines()`', () {
      ...
      }
      {
        ...
        expect(
          ...
          const CodeLines([
            CodeLineSegment(codeLines: [CodeLine('foo'), CodeLine('bar')]),
          ]),
        );
        expect(
        ...
        expect(
          ...
          const CodeLines([
            CodeLineSegment(codeLines: [CodeLine('abc'), CodeLine('foo')]),
          ]),
        );
        expect(
        ...
        expect(
          ...
          CodeLines([
            ...
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 1}'),
              ),
              dirty: true,
            ),
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 2}'),
              ),
              dirty: true,
            ),
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 3}'),
              ),
              dirty: true,
            ),
          ]),
        ...
        expect(
          ...
          CodeLines([
            ...
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 4}'),
              ),
              dirty: true,
            ),
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 5}'),
              ),
              dirty: true,
            ),
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 6}'),
              ),
              dirty: true,
            ),
            CodeLineSegment(
              codeLines: List.generate(
                256,
                (index) => CodeLine('${index + 512 + 256 * 7}'),
              ),
              dirty: true,
            ),
          ]),
